  ...
  constructor() {
    ...
    refesh = () => {
      ...
      for (let i = localData.length - 1; i < localData.length; i += 1) {
        ...
        const texte = document.createTextNode(localData[i]);
    ...
    remove = (event) => {
      if (event.target.classList[0] === 'tester') {
        this.storeData = JSON.parse(localStorage.getItem('dataList'));
        const cont = document.getElementById(`paragraph${event.target.id}`);
        document.getElementById((`list-item${event.target.id}`)).remove();
        const rem = this.storeData.filter((letter) => letter !== cont.textContent);
        this.storeData = rem;
        localStorage.setItem('dataList', JSON.stringify(this.storeData));
      }
    }
    ...
    itemColor = (event) => {
      if (event.target.className === 'paragraph2') {
        ...
        const newId = parent.childNodes[1].id;
        const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
        const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
